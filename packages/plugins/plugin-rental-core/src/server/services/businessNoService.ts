...
/**
 * generateBusinessNo：服务方法骨架。
 * 中文说明：对应 docs/service-methods.md 中的 `generateBusinessNo`，当前只保留签名和 TODO，不实现完整业务逻辑。
 * 输入参数类型：`prefix: string`。
 * 返回类型：`Promise<ServiceResult<{ businessNo: string }>>`。
 ...
 */
export async function generateBusinessNo(prefix: string): Promise<ServiceResult<{ businessNo: string }>> {
  void [prefix];
  throw new Error('TODO: generateBusinessNo 仅为骨架，尚未接入真实业务实现。');
}
