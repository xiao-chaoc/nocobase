...
/**
 * manuallyAdjustRentDate：服务方法骨架。
 * 中文说明：对应 docs/service-methods.md 中的 `manuallyAdjustRentDate`，当前只保留签名和 TODO，不实现完整业务逻辑。
 * 输入参数类型：`ledgerId: ID, adjustmentType: ManualAdjustmentType, reason: string`。
 * 返回类型：`Promise<ServiceResult<{ adjustmentId: ID }>>`。
 ...
 */
export async function manuallyAdjustRentDate(
  ledgerId: ID,
  adjustmentType: ManualAdjustmentType,
  reason: string,
): Promise<ServiceResult<{ adjustmentId: ID }>> {
  void [ledgerId, adjustmentType, reason];
  throw new Error('TODO: manuallyAdjustRentDate 仅为骨架，尚未接入真实业务实现。');
}
