...
默认不生产，只允许隔离 PostgreSQL 测试库。脚本会执行已存在的一键隔离阶段；Runtime、Permission、Page/menu/block、Mock data import 和 Business smoke test 当前以 codex_dry_run / codex_mock_report 记录，真实本地执行仍属于 pre-release local execution；尚未实现的阶段会记录 skipped，并进入 modification_items。
USAGE
}
...
本报告由隔离总测试脚本生成。它不代表生产就绪；当前 production_ready=false。Runtime、Permission、Page/menu/block、Mock data import 和 Business smoke test 阶段当前记录为 codex_dry_run / codex_mock_report，真实本地执行仍标记为 pre-release local execution。阶段脚本不存在时会被标记 skipped，并进入修改项清单。
